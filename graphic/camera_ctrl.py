...
class ArcballCamera:
    def __init__(self, view_widget):
        ...
        # Call set_model_scale() after every draw_model() so the camera
        # always knows how big the structure is.
        self._model_scale = 1.0          # bounding-box diagonal, world units
        ...
        # ── Single-use pan mode (toolbar button) ──────────────────────────
    ...
    def rotate(self, dx, dy):
        ...
        """
        SENSITIVITY = 0.35          # degrees per pixel — feels like SAP2000

        new_el = self.view.opts['elevation'] + dy * SENSITIVITY
        self.view.opts['elevation'] = max(-89.0, min(89.0, new_el))
    ...
    def pan(self, dx, dy, width, height):
        ...
        scale = visible_h / height          # world units per pixel
    ...
    def zoom(self, delta, mouse_x, mouse_y, width, height):
        ...
        min_dist   = self._model_scale * 0.02   # 2 % of model size
        fly_thresh = self._model_scale * 0.12   # 12 % of model size

        # ── Fly-through mode (very close zoom) ────────────────────────────
        if dist < fly_thresh and delta > 0:
            view_vec = self._view_direction()
            step = max(dist * 0.15, min_dist * 0.5)   # proportional step
            self.view.opts['center'] = center + (view_vec * step)
            self.view.update()
            return

        # ── Normal zoom ───────────────────────────────────────────────────
        # Slightly gentler than before (0.88 / 1.14) for more control
        ...
        # Zoom toward cursor (pan center toward mouse offset)
        ...
        self.view.opts['distance'] = max(new_dist, min_dist)  # never go inside
        self.view.update()
